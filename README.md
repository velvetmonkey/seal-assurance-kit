...
| Gate receipts in CI | Run `node bin/seal verify fixtures/receipt-block.json` from this kit in a workflow step (P-REF bundled self-check); replace the fixture with your receipt. For spine-v2, add `--receipt-pubkey <64-lowercase-hex>` from an independently provisioned key. Unlike the archived action's stricter profile, P-REF does not require `signed_config`; sufficiency and diff checks are local tools today. |
